...
def collectDataset_linelaser202203(src_dir=r'C:\Users\zk\Desktop\NewAlgo_LineLaser\videos\LineLaser202203',
    ...
    users = {'gd', 'zk', 'fyp', 'qjy'}
    # classes = {'real', 'print', 'laminate', 'laminate_laserline'}
...
def getLineMask(blockBGR):
    gray = cv2.cvtColor(blockBGR, cv2.COLOR_BGR2GRAY)
    h, w = gray.shape[:2]
    ...
    bin[:, w - 2:w] = 0
    bin[h - 2:h, :] = 0
    bin[np.where(gray < 70)] = 0

    return bin


def getMask_Color(block_bgr, sat_threshold=43, val_threshold=46):
    """
    by zk
    获取镭射区中有色区域（高亮高饱和）的掩膜图，有颜色的区域的值置为色调值，无颜色的区域的值置为0
    :param hsv:
    :param flag: 建模未完成时flag为False; 建模完成后flag为True
    :return:
    """
    blockhsv = cv2.cvtColor(block_bgr, cv2.COLOR_BGR2HSV)
    (hue, sat, val) = cv2.split(blockhsv)
...
def get_ColorLine_from_LaserLabel(
    ...
    for cls in class_list:
        ...
        for file in im_files:
            ...
            # linemask: 线条区域
            linemask = getLineMask(im)

            # mask_ColorLine: 彩色线条区域
            # mask_ColorLine = getMask_ColorLine(im, linemask)
            mask_Color = getMask_Color(im, 70, 70)
            mask_ColorLine = cv2.bitwise_and(mask_Color, linemask)
...
if __name__ == "__main__":
    ...
    # collectDataset_linelaser(src_dir)
    # get_ColorLine_from_LaserLabel(src_dir)
    # get_ColorLine_thresh(src_dir=src_dir, area_thresh=300)

    # datacleaner(dataset_dir='ColorLine300')

    # getStats_NonZero(dir_path=r'C:\Users\zk\Desktop\NewAlgo_LineLaser\videos\LineLaser202203\ColorLine500\HardSamples')

    # make trainval Txt for dataset
